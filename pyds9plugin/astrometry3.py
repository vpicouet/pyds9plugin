...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--use_sextractor",
        help="boolean. Use the SourceExtractor program to detect stars, not our built-in program.",
        default=True,
    )
    parser.add_option(
        "--private",
    ...
    if opt.upload or opt.upload_url or opt.upload_xy:
        ...
        if opt.upload_xy:
            ...
            print(opt.upload_xy)
            T = Table.read(opt.upload_xy)
            kwargs.update(
                x=[float(x) for x in T[T.colnames[0]]],
                y=[float(y) for y in T[T.colnames[1]]],
            )
