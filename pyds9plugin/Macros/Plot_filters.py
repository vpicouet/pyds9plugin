...
def PlotSpectraFilters(xpapoint=None, argv=[]):
    ...
    wavelength = m * np.hstack(
        (np.linspace(0, 3e4, 100), np.linspace(1e5, 1e7, 10000))
    )  # np.linspace(0,1e7,int(2e4))
    ...
    spectrum = fnu(
        tab["col2"], tab["col1"]
    )  # tab['col2'] /  tab['col2'].max() / 1.1

    d = {"lam": lam, "spectrum": spectrum, "z": 0}
    spectrumInterp = interpolate.interp1d(
        np.hstack(([0], lam, [1e10])),
        np.hstack(([0], spectrum, [0])),
        kind="linear",
    )
    # New_spectrum = spectrumInterp(wavelength)
    l = ax2.plot(lam, spectrum, "-k", label="Star", linewidth=0.7)
    l_beta = ax2.plot(
        lam[(lam > m * beta_min) & (lam < m * beta_max)],
        spectrum[(lam > m * beta_min) & (lam < m * beta_max)],
        "-b",
        linewidth=0.7,
        label=r"$\beta_{slope}$",
    )
    ...
    for i, line in enumerate(lines):
        if i % 2:
            ...
            y = 1.03
        lines_plot[line] = ax2.plot(
            [m * lines[line], m * lines[line]],
            [y, 0],
            linewidth=0.9,
            c="k",
            linestyle="dotted",
    ...
    for i, filt in enumerate(filts):
        ...
        ax2.fill_between(
            data[0], data[1], alpha=0.4, color=colors[i]
        )  # ,label=labels[i])
        # b = np.linspace(data[0].max()+1,1e9,int(1e6))
        FieldInterp = interpolate.interp1d(
            np.hstack(([0], data[0], [1e10])),
            np.hstack(([0], data[1], [0])),
            kind="linear",
        )
        ...
        mags.append(
            ax1.plot(
                loc,
                10 * spectrumInterp(wavelength).dot(FieldInterp(wavelength)),
                "o",
                c=colors[i],
                markersize=15,
                alpha=0.6,
    ...
    def update(val):
        for line, line_plot in zip(lines, lines_plot):
            lines_plot[line][0].set_xdata(
                [m * (1 + val) * lines[line], m * (1 + val) * lines[line]]
            )
    ...
    def hzfunc(label):
        ...
        l_beta[0].set_data(
            (1 + d["z"]) * lam[(lam > m * beta_min) & (lam < m * beta_max)],
            spectrum[(lam > m * beta_min) & (lam < m * beta_max)],
        )
        l[0].set_label(
            os.path.basename(filename).split(".sed")[0].split(".out")[0]
        )
        ax2.legend(loc="upper right")
        FieldInterp = interpolate.interp1d(
            np.hstack(([0], d["lam"], [1e10])),
            np.hstack(([0], d["spectrum"], [0])),
            kind="linear",
        )
        for mag, filter_, size in zip(mags, filters, sizes):
            mag[0].set_ydata(
