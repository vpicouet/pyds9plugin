...
def EMCCD(
    ...
    TODO: remove too high value at 0
    """
    ...
    if sCIC>0:
        ...
        # cic_disdribution = np.random.exponential(np.power(EmGain, register / n_reg))
        ...
        # gamma_distribution = np.convolve(gamma_distribution,cic_disdribution,mode='same')
        # plt.semilogy(x,cic_disdribution)
        # plt.semilogy(x,gamma_distribution)
        # print(np.sum(cic_disdribution))
        ...
        # plt.semilogy(gamma_distribution)
        # plt.semilogy(x[:len(gamma_distribution)],gamma_distribution)
        # sys.exit()
        # cic_gain = np.power(EmGain, (n_reg/2) / n_reg)
        # cic_disdribution = cic_gain * np.exp(-cic_gain*bins)
        # gamma_distribution = gamma_distribution*(1-p_sCIC) + p_sCIC*np.convolve(gamma_distribution,cic_disdribution[:10],mode='same')
