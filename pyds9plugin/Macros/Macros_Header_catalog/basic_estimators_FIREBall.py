...
table["BottomImage_median"] = np.nanmedian(physical_region[10:480,:]) - table['median_pre_scan']
table["TopImage_median"] = np.nanmedian(physical_region[-30:-10,:]) - table['median_pre_scan']

table["flat"] = (np.nanmedian(physical_region) - table['median_pre_scan'] )/np.nanvar(physical_region)
...
table['long_background'] = np.mean(data[1777:1877, 1929:1947] - table['median_pre_scan'])
...
fits.setval(filename, 'PRESCAN', value=float(table['median_pre_scan']), comment='Median value of the pre scan region')
fits.setval(filename, 'BIAS_OS', value=float(
    table['bias_os']), comment='Bias estimated on overscan region')
...
if full_analysis:
    ...
    [data[1908-20:2006+20, 1908-20:1926+20] - table['median_pre_scan']], name="long_slit_image")
