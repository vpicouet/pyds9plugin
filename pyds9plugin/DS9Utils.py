...
def compute_fluctuation(
    xpapoint, fileOutName=None, ext=1, ext_seg=1, mag_zp=None, sub=None, aper_size=10, verbose=False, plot=False, seg=None, type="image", nomemmap=False, argv=[]):
    ...
    parser.add_argument('-n', '--number_apertures',    default='1000', help='Number of apertures to throw in the image', type=str)#metavar='',
...
def lock(xpapoint=None, argv=[]):
    ...
    parser.add_argument('-l', '--scalelimits',    default='1', help='', type=str)#, choices=['image','none','wcs'])#metavar='',
    parser.add_argument('-s', '--smooth',    default='0', help='', type=str)#, choices=['image','none','wcs'])#metavar='',
    parser.add_argument('-m', '--cmap',    default='1', help='', type=str)#, choices=['image','none','wcs'])#metavar='',
...
def build_wcs_header(xpapoint=None, filename=None, pix_coord=None, increment=None, projection="RA---AIR,DEC--AIR", coord_value=None, argv=[]):
...
def create_DS9regions2(xim, yim, radius=20, more=None, save=True, savename="test", text=0, form="circle", color="green", config=my_conf, DS9_offset=[1, 1], system="image"):
    ...
    if save:
        verboseprint("Saving region file")
        with open(savename + ".reg", "w") as text_file:
...
def fit_gaussian_2d(xpapoint=None, Plot=True, n=300, cmap="twilight_shifted", argv=[]):
    ...
    import numpy as np
    parser = CreateParser(get_name_doc(),path=False)
    parser.add_argument('-p', '--plot',    default='1', help='Interactive plot for gaussian fitting', type=str)#metavar='',
    args = parser.parse_args_modif(argv,required=False)
...
def throughfocus_(
    ...
    t = Table(
        names=("name", "number", "t", "x", "y", "Sigma", "EE50", "EE80", "Max pix", "Flux", "Var pix", "Best sigma", "Best EE50", "Best EE80", "Best Maxpix", "Best Varpix"),
        dtype=("S15", "f4", "f4", "f4", "f4", "f4", "f4", "f4", "f4", "f4", "f4", "f4", "f4", "f4", "f4", "f4"),
    )
    t.add_row(
        (
...
def explore_throughfocus(xpapoint, argv=[]):
...
def radial_profile(xpapoint=None, Plot=True, config=my_conf, center_type=None, fibersize=None, log=None, argv=[]):
    ...
    parser.add_argument('-d', '--source_diameter',    default=1, help='Fiber diameter in pixel. 0 for pure gaussian fitting, else data will be fitted by (Disk * Gaussian)', metavar='',type=str)
    args = parser.parse_args_modif(argv,required=False)
...
def Charge_path_new(filename, entry_point=3, entry=None, All=0, begin="-", end="-", liste="-", patht="-", config=my_conf):
    ...
    if len(globglob(patht)) > 0:
        ...
        if liste.split("-")[0] != "":
            ...
            numbers = [n1, n2]
            files = np.sort(glob.glob("%s%s%s" % (filen1, "?" * n, filen2)))
            # print(files)
            # print(files)
            im_numbers = []
            for file in files:
    ...
    if len(path) == 0:
        d = DS9n()
        d.set("analysis message {Could not find any image, the DS9 loaded image is in the right folder and have a comparable name?}")
    return path
...
def throughfocus_visualisation(xpapoint, argv=[]):
...
def throw_apertures(xpapoint=None, argv=[]):
    ...
    import numpy as np
    parser = CreateParser(get_name_doc())
    parser.add_argument('-a', '--aperture',    default='10,10', help='Aperture radius in pixels', type=str)#metavar='',
...
def throughslit(xpapoint=None, DS9backUp=DS9_BackUp_path, config=my_conf, argv=[]):
    """Perform a throughslit analysis and return the centered image
...
def DS9Update_old(xpapoint=None, Plot=True):
    """Always display last image of the repository and will upate with new ones
    """
    import time
...
def import_table_as_region(xpapoint=None, name=None, x="x", y="y", ID=None, system="image", form="circle", size=10, wcs=False, query="-", argv=[]):
    ...
    parser.add_argument('-s', '--selection', help='selection of the region in th catalog: Use | for OR and \& for AND', type=str,metavar='')#metavar='',
...
def mask_regions(xpapoint=None, length=20, argv=[]):
    ...
    cosmicRays = save_region_as_catalog(xpapoint, new_name="/tmp/cat.csv")
    cosmicRays["front"] = 1
    cosmicRays["dark"] = 0
...
def autocorrelation(xpapoint=None, argv=[]):
    """Computes image(s) 2D autocorrelation
    """
    parser = CreateParser(get_name_doc(),path=True)
    parser.add_argument('-t', '--type',    default='2D-XY', help='', type=str, choices=['2D-XY','X','Y'])#metavar='',
    args = parser.parse_args_modif(argv)

    d = DS9n(xpapoint)
    filename = globglob(args.path)#getfilename(d)
...
def FFT(xpapoint=None, config=my_conf, argv=[]):
    """Compute FFT on given image(s)
    """
    parser = CreateParser(get_name_doc(),path=True)
    args = parser.parse_args_modif(argv)
...
def create_header_catalog(xpapoint=None, files=None, info=False, config=my_conf, argv=[]):
    ...
    info, extension = '','-'
    try:
        verboseprint("info, extension = ", info, extension)
        if info == "1":
            info = True
        elif info == "0":
            info = False
...
def CreateCatalogInfo(t1, verbose=False, config=my_conf, write_header=True):
    ...
    if len(error_cat) > 0:
        try:
            ...
            file.close()
            d = DS9n()
            d.set("analysis message yesno {At least one image here is not corresponding to its header}")
        except:
            pass
    return new_cat
...
def convert_image(xpapoint=None,path=None,argv=[]):
    ...
    parser = CreateParser(get_name_doc(),path=True)
    parser.add_argument('-t', '--type',    default='8,uint8', help='Conversion type of the image', type=str, choices=['8,uint8','16,int16','32,int32','64,int64','-32,float32','-64,float64'])#metavar='',
...
def fill_regions(xpapoint, argv=[]):
    ...
    parser = CreateParser(get_name_doc())
    parser.add_argument('-v', '--value', help='Value to replace in the regions', type=str)#metavar='',
    parser.add_argument('-o', '--overwrite',    default='1', help='Overwrite the image', type=str)#metavar='',
    args = parser.parse_args_modif(argv)
...
def interpolate_nans(xpapoint, argv=[]):
    """Replace NaNs value in given image(s) by values interpolated around NaNs value
    """
    parser = CreateParser(get_name_doc(),path=True)
    args = parser.parse_args_modif(argv,required=True)
...
def background_estimation(xpapoint=None, n=2, DS9backUp=DS9_BackUp_path, Plot=True, argv=[]):
    ...
    import numpy as np
    parser = CreateParser(get_name_doc(),path=True)
    parser.add_argument('-s', '--sigma',    default='3', help='', type=str)#metavar='',
    parser.add_argument('-b', '--background',    default='MeanBackground', help='Different background estimators that can be used', type=str, choices=['MeanBackground','MedianBackground','ModeEstimatorBackground','MMMBackground','SExtractorBackground','BiweightLocationBackground'])#metavar='',
    ...
    parser.add_argument('-f', '--filter',    default='3,3', help='The window size of the 2D median filter to apply to the low-resolution background map', type=str)#metavar='',
    parser.add_argument('-box', '--box',    default='40,40', help='Size of the box to be used', type=str)#metavar='',
    parser.add_argument('-per', '--percentile',    default='20', help='Percentile of the image to exclude in the background estimation', type=str)#metavar='',
    parser.add_argument('-snr', '--snr',    default='3', help='The snr per pixel above the background for which to consider a pixel as possibly being part of a source', type=str)#metavar='',
    parser.add_argument('-n', '--npixels',    default='3', help='The number of connected pixels, each greater than threshold, that an object must have to be detected.', type=str)#metavar='',
    parser.add_argument('-d', '--dilate',    default='5', help='The size of the square array used to dilate the segmentation image.', type=str)#metavar='',
...
def fit_ds9_plot(xpapoint=None, config=my_conf, exp=False, double_exp=False, double_schechter=False, schechter=False, Type="Linear", EMCCD_=False, nb_blackbody=0, argv=[]):
    """Fit interactively any DS9 plot with different fucntions [DS9 required]
    """
    import matplotlib.pyplot as plt
    from matplotlib.widgets import CheckButtons  # RadioButtons,
    import numpy as np
    from astropy.table import Table
    parser = CreateParser(get_name_doc(),path=True)
    parser.add_argument('-b', '--background',    default='1', help='Background to fit', type=str, choices=['Constant','None','slope','Exponential','DoubleExponential'])#metavar='',
...
def manual_fitting(xpapoint=None, initial="a+b*max(ydata)*exp(-(x-c*x[argmax(ydata)])**2/len(ydata)/d)"):
...
def get_depth_image(xpapoint=None, argv=[]):
    ...
    """
    parser = CreateParser(get_name_doc(),path=True)
    parser.add_argument('-a', '--aperture',    default='2', help='Aperture radius in pixels', type=str)#metavar='',
    parser.add_argument('-z', '--zero_point_magnitude',    default='0', help='Zero point magnitude of the image', type=str)#metavar='',
    parser.add_argument('-n', '--number_apertures',    default='1000', help='Number of apertures to throw in the image', type=str)#metavar='',
    args = parser.parse_args_modif(argv,required=True)
...
def RunSextractor(xpapoint=None, filename=None, detector=None, path=None, argv=[]):
    ...
    if DETECTION_IMAGE is not None:
        command = (
            ...
            + DETECTION_IMAGE
            + filename
            + " -WRITE_XML Y   -XML_NAME /tmp/%s.xml -" % (os.path.basename(filename))
...
def Convertissor(xpapoint=None, argv=[]):
    ...
    from decimal import Decimal
    parser = CreateParser(get_name_doc())
    parser.add_argument('-v', '--value',    default='1', help='Value to convert', type=str)#metavar='',
    parser.add_argument('-u', '--unit1',    default='angstrom', help='Unit1', type=str, choices=['angstrom','meter','parsec','astronomical_unit','lightyear','Joule','erg','Ry','cal','eV','','kg','earthMass','M_e','M_p','Msun','lb','oz'])#metavar='',
    parser.add_argument('-U', '--unit2',    default='meter', help='Unit1', type=str, choices=['angstrom','meter','parsec','astronomical_unit','lightyear','Joule','erg','Ry','cal','eV','','kg','earthMass','M_e','M_p','Msun','lb','oz'])#metavar='',
...
def open_file(xpapoint=None, filename=None, argv=[]):
    """Open file(s) in DS9 in an easier way
    ...
    parser.add_argument('-t', '--type',    default='Slice', help='Way to open the images', type=str)#metavar='',
    parser.add_argument('-c', '--clip',    default='0', help='Open screenshot image from mac', type=str)#metavar='',
    args = parser.parse_args_modif(argv)
...
%i/%i - Copy the path you want to use and hit n when you are ready."""
    ...
    WaitForN(xpapoint)
    vebroseprint("""%i/%i - Paste the path in the Regexp Path entry.""" % (i, n), verbose="1")
    i += 1
...
def test_suite(xpapoint=None, argv=[]):
    """Test suite: run several fucntions in DS9
    """
    from subprocess import Popen
    parser = CreateParser(get_name_doc())
    parser.add_argument('-t', '--tutorial', help='Name of the tutorial to launch', type=str,metavar='',choices=['1-Generic-Tools','2-Photometric-Analysis','3-Image-Quality-Assesment','4-Image-Processing','5-All-In-One'])#metavar='',
    args = parser.parse_args_modif(argv,required=False)

    Popen([" DS9Utils %s Button" % (xpapoint)], shell=True, stdin=None, stdout=None, stderr=None, close_fds=True)
...
def python_command(xpapoint=None,argv=[]):
    ...
    parser = CreateParser(get_name_doc(),path=True)
    parser.add_argument('-e', '--exp',    default='ds9', help='Expression to process . eg ds9+=1', type=str,metavar='')
    parser.add_argument('-r', '--path_to_remove',    default='/tmp/test.fits', help='Path of a second image to use with', metavar='',type=str)
    parser.add_argument('-o', '--overwrite',    default='0', help='Overwrite image', metavar='')
    args = parser.parse_args_modif(argv,required=True)

    #filename = getfilename(d)
#    path2remove, exp, eval_ = sys.argv[3:6]
    path2remove, exp, eval_ = args.path_to_remove, args.exp, 0
...
def main():
    ...
    DictFunction_Generic = {
        ...
        "fit_gaussian_2d": fit_gaussian_2d,#fit_gaussian_2d
        "organize_files": organize_files,#organize_files
        "import_table_as_region": import_table_as_region,#import_table_as_region
        "add_field_to_header": add_field_to_header,#add_field_to_header
        "fit_ds9_plot": fit_ds9_plot,#fit_ds9_plot
        "test_suite": test_suite,#test_suite
        #"Convolve2d": Convolve2d,#convolve
        #"PlotSpectraDataCube": PlotSpectraDataCube,#
        #"StackDataDubeSpectrally": StackDataDubeSpectrally,#
        #"stack_images": stack_images,#stack
        "lock": lock,#lock
        "create_header_catalog": create_header_catalog,#create_header_catalog
        "python_command": python_command,#python_command
        "save_region_as_catalog": save_region_as_catalog,#save_region_as_catalog
        "mask_regions": mask_regions,#mask_regions
        "CreateImageFromCatalogObject": CreateImageFromCatalogObject,#sextractor?
        "plot_3d": plot_3d,#plot_3d
        "original_settings": original_settings,#original_settings
        "next_step": next_step,#
        "background_estimation": background_estimation,#background_estimation
        "verbose": verbose,#
        # "build_wcs_header": build_wcs_header,#build_wcs_header
        "open_file": open_file,#open_file
        #"checkFile": checkFile,#
        "Button": Button,#
        "throw_apertures": throw_apertures,#throw_apertures
        "CreateContourRegions": CreateContourRegions,#
        "convert_image":convert_image,#convert_image
        "manual_fitting":manual_fitting,
        "compute_gain":compute_gain,
    }  # ,'NextButton':NextButton

    DictFunction_AIT = {
        "center_region": center_region,#center_region
        ...
        "interpolate_nans": interpolate_nans,#interpolate_nans
        "trim": trim,#trim
        "column_line_correlation": column_line_correlation,#column_line_correlation
        #"ComputeEmGain": compute_gain,#compute_gain
        # "FFT": FFT,#FFT
        # "autocorrelation": autocorrelation,#autocorrelation
        "get_depth_image": get_depth_image,#depth
        "emccd_model": emccd_model,#emccd_model
        # "analyze_fwhm": analyze_fwhm,#analyze_fwhm
    }

    DictFunction_SOFT = {
        "DS9SWARP": DS9SWARP,#swarp
        "DS9PSFEX": DS9PSFEX,#psfex
        ...
        "interactive_plotter": interactive_plotter,#interactive_plotter
        "PlotSpectraFilters": PlotSpectraFilters,#plot_filters
        "sextractor_pp": sextractor_pp,#sextractor_pp
        "maxi_mask": maxi_mask,#maxi_mask
    ...
    if (len(sys.argv) == 1) | (len(sys.argv) == 2) & (sys.argv[-1] in ["help",'h','-h','load','l','-l']):
        CreateFolders(DS9_BackUp_path=os.environ["HOME"] + "/DS9QuickLookPlugIn/")
        PresentPlugIn()
        for function in DictFunction:
            if function not in ["verbose", "next_step",'Quit','checkFile','Button','CreateContourRegions',"mask_regions"]:  # ,'setup'
